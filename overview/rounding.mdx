...
Totals in GOBL will **always** be presented using the currency's default precision. This is important as most tax agencies do not recognize a level of precision that cannot be dealt with using the local coinage. The way the calulations are made is important.
...
An often unexpected consequence of the "preciese" rounding method however is that sometimes the presented numbers might not add up and be a few cents out. For example, take the following invoice lines:
