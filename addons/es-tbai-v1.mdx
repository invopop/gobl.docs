...
Bizkaia's LROE Modelo 140 register. Required for individual suppliers in
Bizkaia, but not for organisations, who file through Modelo 240.

The value must be a numeric code of up to 7 digits, matching the pattern
`^\d{1,7}$`. The full list of valid codes is published by the Bizkaia tax
authority.
...
| ----- | ----- |
| Pattern | `^\d{1,7}$` |
| Required for | Individual suppliers in Bizkaia (region `BI`) |
| Sources | [LROE list of activity codes](https://www.batuz.eus/fitxategiak/batuz/lroe/batuz_lroe_lista_epigrafes_v1_0_4.xlsx) · [Modelo 140 specification](https://www.batuz.eus/fitxategiak/batuz/lroe/lroe_140_v_1_0.pdf) |
</Accordion>

Set the activity code on the supplier's extensions:
