...
Most addons are bundled with the GOBL library itself, but some are maintained in their own Go modules — for example, the French CTC addons live in [gobl.fr.ctc](https://github.com/invopop/gobl.fr.ctc). GOBL core keeps a curated list of approved external addons so that their keys remain valid `$addons` values in the JSON Schema, but recognition is not the same as availability: a document declaring an external addon can only be calculated or validated by an application that imports the addon's module, which registers its extensions, normalization, and validation rules.
