...
| Category | `category` | [cbc.cbc.Key](/draft-0/cbc/cbc._key) | Category helps classify the link according to a fixed list. This is optional but highly recommended as it helps receivers better understand the purpose of the link and potentially how it should be presented. |
|  | `key` | [cbc.cbc.Key](/draft-0/cbc/cbc._key) | Key is a unique identifier for the link within the header and category if provided. |
| Code | `code` | [cbc.cbc.Code](/draft-0/cbc/cbc._code) | Code used to identify the contents of the link. |
...
| Digest | `digest` | [dsig.dsig.Digest](/draft-0/dsig/dsig._digest) | Digest is used to verify the integrity of the destination document when downloaded from the URL. |
| URL | `url` | string | URL of the resource. |
| Metadata | `meta` | [cbc.cbc.Meta](/draft-0/cbc/cbc._meta) | Meta contains additional metadata about the link. |
