---
title: ScenarioSet
comment: Generated with GOBL v0.38.0
---

ScenarioSet is a collection of tax scenarios for a given schema that can be used to determine special codes or notes that need to be included in the final document.

## Schema ID

`https://gobl.org/draft-0/tax/regime#/$defs/ScenarioSet`

## Properties

| Property | Type                                           | Req. | Description                                          |
|----------|------------------------------------------------|------|------------------------------------------------------|
| `schema` | string                                         | true | Partial or complete schema URL for the document type |
| `list`   | array of [tax.Scenario](/draft-0/tax/scenario) | true | List of scenarios for the schema                     |

