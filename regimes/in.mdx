...
| ---- | ----- | ---- | ----------- |
| <code>GOBL-IN-ORG-IDENTITY-01</code> | code | type in [PAN]; matches ^[A-Z]{5}[0-9]{4}[A-Z]$ | identity code must be a valid PAN format |
| <code>GOBL-IN-ORG-IDENTITY-02</code> | code | type in [HSN]; matches ^(?:\d{4}|\d{6}|\d{8})$ | identity code must be a valid HSN format |
</Accordion>
<Accordion title="org.Item">

| Code | Field | Test | Description |
| ---- | ----- | ---- | ----------- |
| <code>GOBL-IN-ORG-ITEM-01</code> | identities | context: regime in [IN]; has a type in [HSN] | all items must have an HSN identity code |
</Accordion>
<Accordion title="tax.Identity">

| Code | Field | Test | Description |
| ---- | ----- | ---- | ----------- |
| <code>GOBL-IN-TAX-IDENTITY-01</code> | code | code in [IN]; valid | invalid Indian tax identity code |
</Accordion>
</AccordionGroup>