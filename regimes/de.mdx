...
| ---- | ----- | ---- | ----------- |
| <code>GOBL-DE-BILL-INVOICE-01</code> | supplier | context: regime in [DE]; has tax ID code or identity with 'de-tax-number' key | invoice DE supplier must have either tax ID code or identity with 'de-tax-number' key |
</Accordion>
<Accordion title="org.Identity">

| Code | Field | Test | Description |
| ---- | ----- | ---- | ----------- |
| <code>GOBL-DE-ORG-IDENTITY-01</code> | code | is tax number; matches ^(\d{2}/\d{3}/\d{5}|\d{3}/\d{3}/\d{5}|\d{3}/\d{4}/\d{4})$ | German tax number code must be in valid format |
</Accordion>
<Accordion title="tax.Identity">

| Code | Field | Test | Description |
| ---- | ----- | ---- | ----------- |
| <code>GOBL-DE-TAX-IDENTITY-01</code> | code | code in [DE]; valid | invalid German VAT identity code |
</Accordion>
</AccordionGroup>